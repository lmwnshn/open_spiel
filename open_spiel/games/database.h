...
// WAN 2021: Modified from tic_tac_toe.h.
...
class Tuner {
 public:
  ...
  [[nodiscard]] const std::vector<TuningAction> &GetTuningActions() const { return actions_; }
...
class DatabaseState : public State {
 public:
  DatabaseState(std::shared_ptr<const Game> game);

  DatabaseState(const DatabaseState &) = default;
  DatabaseState &operator=(const DatabaseState &) = default;
...
#endif  // OPEN_SPIEL_GAMES_DATABASE_H